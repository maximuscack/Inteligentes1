# Lista de campos para el semáforo
CAMPOS_SEMAFORO = ['habilitado', 'en_rojo', 'tiempo_por_estado']

# Ruta al archivo de grafo de la ciudad de Manizales en formato GraphML
CIUDAD_MANIZALES_GRAPHML = 'app/data/mapas/manizales.graphml'


# Directorio donde se almacenan los mapas de la ciudad de Manizales
CIUDAD_MANIZALES_DIR = 'app/data/mapas/'

# Directorio donde se almacenan las imágenes
IMAGES_DIR = 'app/data/images'

# Lista de nombres de barrios en la ciudad de Manizales
NOMBRES_BARRIOS = [
    'Comuna Cumanday', 'Comuna La Macarena',
    'Comuna Palogrande', 'Comuna La Fuente',
    'Comuna San José', 'Comuna Nuevo Horizonte',
    'Comuna Atardeceres', 'Comuna Ciudadela del Norte',
    'Comuna Ecoturística Cerro de Oro', 'Comuna Universitaria',
]
